...
//mongo db connection -------------------
const uri = 'mongodb+srv://21pa1a0562:Ke0kwJpMFNCcwCSw@cluster0.717asxo.mongodb.net/?retryWrites=true&w=majority'
...
app.post("/adminsubmit", async (req, res) => {
  ...
  if (email === "jogikarthik123@gmail.com" && password === "karthik7410") {
...
app.get("/usersEnrolled", async (req, res) => {
  try {
    const enrolledCourses = await enrolls.find();
    const courseDetails = {};
    for (const course of enrolledCourses) {
      const courseinfo = await allCourses.findOne({ courseTitle: course.courseTitle });
    ...
    try {
      ...
      if (user) {
        res.render("usersEnrolled", { courseDetails })
      } else {
        res.redirect('/studentSignUp')
      }
    } catch (err) {
